...
function creerImageFallback(outputPath, segIndex) {
  return new Promise((resolve) => {
    if (!ffmpeg) {
      // Sans FFmpeg : JPEG minimal valide 1x1 pixel noir
      // Ce JPEG 1x1 est validé par la spec JFIF
      const j = Buffer.from([
      ...
      try { fs.writeFileSync(outputPath, j); } catch(e2) {}
      resolve(); return;
    }
    // Avec FFmpeg : générer une vraie image 1080x1920 avec couleur de fond
    const idx   = (segIndex || 0) % FALLBACK_COLORS.length;
    const color = FALLBACK_COLORS[idx];
    ffmpeg()
      .input('color=c=' + color + ':size=1080x1920:rate=1')
      .inputOptions(['-f', 'lavfi'])
      .outputOptions(['-frames:v', '1', '-q:v', '2'])
      .output(outputPath)
      .on('end', resolve)
      .on('error', function() {
        // Ultime fallback : JPEG 1x1 valide
        const j = Buffer.from([0xFF,0xD8,0xFF,0xE0,0x00,0x10,0x4A,0x46,0x49,0x46,0x00,0x01,0x01,0x00,0x00,0x01,0x00,0x01,0x00,0x00,0xFF,0xD9]);
        try { fs.writeFileSync(outputPath, j); } catch(e3) {}
        resolve();
      })
      .run();
  });
